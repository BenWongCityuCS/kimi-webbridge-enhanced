...
- Script: **`scripts/webbridge_tab_limit_proxy.py`**, inside this skill folder. Start it in the background, e.g.
  `python "<skill folder>/scripts/webbridge_tab_limit_proxy.py"`
- It listens on `http://127.0.0.1:11086` and forwards **everything** to the real daemon on `:10086`.
- Guard: a `navigate` with `newTab: true` first counts the tabs in *that session*; if it is already ≥ 3 the request is refused with **HTTP 429** and `{"ok": false, "error": {"code": "tab_limit_exceeded", "message": "当前 session '…' 已有 N 个标签页，必须关闭至少一个 tab（调用 close_tab）低于 3 个后，才能新建网页。"}}`. All other actions (`snapshot`, `click`, `fill`, `evaluate`, `close_tab`, `list_tabs`, …) pass through untouched.
- So: give each subagent its **own session name** and point it at `http://127.0.0.1:11086/command`. Counts are per session, so agents never block each other; on a 429 the agent should `close_tab` and retry.
